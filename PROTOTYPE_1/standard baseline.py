...
SYSTEM_PROMPT = (
    ...
    "Answer questions about the CpE curriculum, prerequisites, and academic policies "
    "as accurately as you can based on your training knowledge. "
...
def generate(client: dict, model_id: str,
    ...
    # No-RAG: question sent directly with no retrieved context
    user_msg = question
...
def run_evaluation(model_label: str, model_id: str, config: dict,
    ...
    for i, test in enumerate(TEST_CASES):
        ...
        t1       = time.time()
        answer   = generate(client, model_id, test["question"], config)
...
if __name__ == "__main__":
        ...
        for param_config in PARAM_CONFIGS:
            try:
                summary = run_evaluation(
                    label, model_id, param_config, client
                )
    ...
    if all_summaries:
        ...
        with open("no_rag_results.json", "w", encoding="utf-8") as f:
            json.dump(all_summaries, f, indent=2, ensure_ascii=False)
        print(f"\nFull results saved to no_rag_results.json")
