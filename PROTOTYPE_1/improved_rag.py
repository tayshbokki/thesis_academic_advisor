"""
IMPROVED RAG — Hybrid Search (BM25 + Dense) + Reranking
SO3 objectives:
...
Improvements over Naive RAG:
  ...
  - top_k increased from 3 to 5 for wider retrieval net
"""
...
# OBJECTIVE 5 (RETRIEVAL): TOP-K AND ALPHA CONFIGS TO TUNE
# top_k = number of candidate chunks fetched before reranking
# alpha = weight for dense score (0.0 = BM25 only, 1.0 = dense only)
RETRIEVAL_CONFIGS = [
    {"top_k": 4,  "alpha": 0.5, "label": "k=4 a=0.5"},
    {"top_k": 6,  "alpha": 0.5, "label": "k=6 a=0.5"},   # default
    {"top_k": 8,  "alpha": 0.5, "label": "k=8 a=0.5"},
    {"top_k": 6,  "alpha": 0.3, "label": "k=6 a=0.3"},   # BM25-heavy
    {"top_k": 6,  "alpha": 0.7, "label": "k=6 a=0.7"},   # dense-heavy
    {"top_k": 6,  "alpha": 1.0, "label": "k=6 a=1.0"},   # dense only (ablation)
    {"top_k": 6,  "alpha": 0.0, "label": "k=6 a=0.0"},   # BM25 only (ablation)
]
...
def retrieve_hybrid(collection, query: str, top_k: int = 5,
                    alpha: float = 0.5) -> str:
    """
    Hybrid retrieval: combines dense (ChromaDB) + sparse (BM25)
    alpha = weight for dense score (0.0 = BM25 only, 1.0 = dense only)
    ...
    for doc in all_docs:
        ...
        combined[doc] = alpha * d_score + (1 - alpha) * b_score
...
def generate(client: dict, model_id: str, context: str,
    ...
    if client["provider"] == "hf":
        ...
        # DeepSeek R1 sometimes returns None content — handle gracefully
        content = response.choices[0].message.content
        if content is None:
            content = getattr(response.choices[0].message, 'reasoning_content', None)
        if content is None:
            content = "I don't have that information — please consult your adviser."
...
def run_evaluation(model_label: str, model_id: str, config: dict,
                   collection: chromadb.Collection, client: dict,
                   retrieval_config: dict = None) -> dict:

    # Use provided retrieval config or fall back to defaults
    if retrieval_config is None:
        retrieval_config = {"top_k": 6, "alpha": 0.5, "label": "k=6 a=0.5"}

    top_k = retrieval_config["top_k"]
    alpha = retrieval_config["alpha"]
    r_label = retrieval_config["label"]

    print(f"\n  --- Config: {config['label']} | Retrieval: {r_label} ---")
    ...
    for i, test in enumerate(TEST_CASES):
        ...
        t0         = time.time()
        raw_chunks = retrieve_hybrid(collection, test["question"],
                                     top_k=top_k, alpha=alpha).split("\n\n")
        reranked   = rerank(test["question"], raw_chunks, top_k=min(4, top_k))
        context    = "\n\n".join(reranked)
    ...
    print(f"  SUMMARY [{config['label']} | {r_label}]")
...
if __name__ == "__main__":
    ...
    if os.getenv("GEMINI_API_KEY"):
        try:
            from google import genai as google_genai
            # Just store the key — client is created per call
    ...
    for model_config in MODELS_TO_TEST:
        ...
        for param_config in PARAM_CONFIGS:
            for ret_config in RETRIEVAL_CONFIGS:
                try:
                    summary = run_evaluation(
                        label, model_id, param_config,
                        collection, client,
                        retrieval_config=ret_config
                    )
                    all_summaries.append(summary)
                except Exception as e:
                    print(f"  [SKIPPED] {param_config['label']} | {ret_config['label']} — {e}\n")
                    continue
    ...
    if all_summaries:
        ...
        print(f"{'='*100}")
        print(f"{'Model + LLM Config + Retrieval':<55} {'R-L':>6} {'BERT':>7} {'METEOR':>7} {'Halluc':>8} {'AvgTime':>8} {'<5s':>5}")
        print("-" * 115)

        for s in sorted(all_summaries, key=lambda x: -x["avg_rouge_l"]):
            name = f"{s['model']} [{s['config']}] [{s.get('retrieval_config','k=6 a=0.5')}]"
            print(
                f"{name:<55} "
                f"{s['avg_rouge_l']:>6.3f} "
                f"{s.get('avg_bert_score', 0):>7.3f} "
                f"{s['avg_meteor']:>7.3f} "
        ...
        with open("improved_rag_results.json", "w", encoding="utf-8") as f:
            json.dump(all_summaries, f, indent=2, ensure_ascii=False)
        print(f"\nFull results saved to improved_rag_results.json")
